...
void obtenerDatos(istream &file,int nlines,int n,int mode,item* item_list){
    ...
    for(int i=0;i<n;i++){
        ...
        case INT_FLOAT:
            int e0; float r0,r1;
            file >> e0 >> r0;
            item_list[i].setValues(0,0,0,e0,0,0,0,0,r1); //solo valores en el nodo 1
            break;
        case INT_FLOAT_FLOAT:
            int e; float r,rr,rrr;
            file >> e >> r >> rr >> rrr;
            item_list[i].setValues(e,r,rr,rrr,0,0,0,0,0); //id, x, y, z. rrr es z
            break;
        case INT_INT_INT_INT:
            int e1,e2,e3,e4,e5;
            file >> e1 >> e2 >> e3 >> e4 >> e5 ;
            item_list[i].setValues(e1,0,0,0,e2,e3,e4,e5,0); //id, nodo1, nodo2, nodo 3, e5 es el nodo 4
            break;
        }
    }
}
